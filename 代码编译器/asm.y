...
%token ADD SUB MUL DIV TST STO LOD JMP JEZ JLZ JGZ DBN DBS IN OUTN OUTC OUTS NOP END
%token AND OR
%token <number> INTEGER REG
%token <float_val> FLOAT
%token <string> LABEL
%token ARR_LOAD ARR_STORE
%%
...
;
arr_load_stmt : ARR_LOAD REG ',' '(' REG ')' 
{
    byte2(I_LOD_4);
    byte1($2);
    byte1($5);
...
;
arr_store_stmt : ARR_STORE '(' REG ')' ',' REG 
{
    byte2(I_STO_1);
    byte1($3);
    byte1($6);
...
in_stmt : IN { byte2(I_IN_I); byte1(0); byte1(0); byte4(0);}
;

out_stmt : OUTN { byte2(I_OUT_I); byte1(0); byte1(0); byte4(0);}
| OUTC { byte2(I_OUT_C); byte1(0); byte1(0); byte4(0);}
